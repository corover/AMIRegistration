...
function District() {
  ...
  const getLabelFromValue = (value: any) => {
    const date = apiData.district.find(
  ...
  const filteredStatesOptions =
    apiData &&
    apiData.district.length > 0 &&
    apiData.district.filter((val: any) =>
  ...
  useEffect(() => {
    ...
    if (
      transcript.length > 0 &&
      check &&
      Array.isArray(apiData.district) &&
      apiData.district.some((option: any) =>
  ...
  React.useEffect(() => {
    if (apiData && apiData.district.length < 5) {
                      ...
                      <div className="optionContainer" key={index}>
                        <p
                          ...
                          onClick={() => {
                            ...
                            registerFlow(stateID, nextContext);
                            setCheck(false);
                          }}
                        >
                          {val.name}
